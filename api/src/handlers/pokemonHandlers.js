const { crearPokemon, obtenerPokemones, obtenerPokemonPorId, buscarPokemonesPorNombre } = require('../controllers/pokemonControllers');

const crearPokemonHandler = async (req, res) => {
  try {
    const { name, image, hp, attack, defense, speed, height, weight, type } = req.body;
    const newPoke = await crearPokemon(name, image, hp, attack, defense, speed, height, weight, type);
...
module.exports = {
  ...
  obtenerPokemonPorIdHandler,buscarPokemonesPorNombreHandler
};
